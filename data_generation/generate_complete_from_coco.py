...
file_out['info']['description'] = 'Complete Dataset (generated from COCO 2017 Dataset)'
...
for i in range(0, len(file['annotations'])):
    ...
    if file['annotations'][i]['iscrowd'] == 0:
        ...
        # get the keypoints, so long as one is present
        num_keypoints = file['annotations'][i]['num_keypoints']
        keypoints = file['annotations'][i]['keypoints']
        if num_keypoints > 0 and any(k == 2 for k in keypoints[2::3]):
            ...
            cv2.imwrite('data/complete/' + collection + '/' + image_data['file_name'], bmp)

# save new annotation data
with open('data/complete/annotations/person_keypoints_' + collection + '.json', 'w+') as f:
