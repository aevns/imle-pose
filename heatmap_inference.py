...
preds = torch.stack((idx[-1],idx[-2]), dim=-1)
...
if (loss_function == lf.heatmap_target_dkl) :
    plotMultiPosesOnImage(pred_cpu.detach(), logheatmap2image(heatmap_cpu[bestpose]).detach(), ax=axes[1], label='predictions')
else:
