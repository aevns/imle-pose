...
class BasicVector(nn.Module):
    def __init__(self):
        super(BasicVector, self).__init__()

        self.relu = nn.ReLU(inplace=True)

        self.linear1 = nn.Linear(40, 32)
    ...
    def forward(self, x, z): # (3, 64, 48) & (40,)
        block1 = self.conv1(x['image']) + (self.linear1(z).view(-1, 32, 1, 1)) # (32, 32, 24)
        block1 = self.maxpool(self.relu(self.bn1(block1))) # (32, 16, 12)
    ...
    def sample(self, x):
        z = torch.randn((x['image'].shape[0], 40))
        return self.forward(x, z), z
    
    def train_sample(self, x, n):
        for s in range(n):
            z = torch.randn((x['image'].shape[0], 40), device="cuda:0")
